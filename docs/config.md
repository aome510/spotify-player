...
| `api_rate_limit_retries`          | Number of times to retry a GET request after Spotify returns `429 Too Many Requests`.                | `2`                                                                    |
...
- When Spotify returns `429 Too Many Requests`, `spotify-player` stores the response's `Retry-After` duration globally and retries GET requests up to `api_rate_limit_retries` times. New GET requests wait for an active `Retry-After` period, while mutation requests are never delayed or retried.
