...
pub fn init_playlist_subcommand() -> Command {
    Command::new("playlist")
        ...
        .subcommand(Command::new("import").about("Imports all songs from a playlist into another playlist.")
            ...
            .after_help("Import data for each playlist is stored inside the application's cache folder. If imported again, the command only adds new tracks since last import."))
        ...
        .subcommand(Command::new("fork").about("Creates a copy of a playlist and imports it.")
            .arg(Arg::new("id")
                .value_parser(clap::builder::NonEmptyStringValueParser::new())))
        .subcommand(Command::new("sync").about("Syncs all imported playlists or a single individual import.")
            .arg(Arg::new("id")
                .required(false)
                .value_parser(clap::builder::NonEmptyStringValueParser::new()))
            .arg(Arg::new("delete")
                ...
                .long("delete")
                .action(clap::ArgAction::SetTrue)
                .help("Deletes any previously imported tracks that are no longer in a imported playlist since last import.")))
}
