...
fn improve_query(query: &str) -> String {
    // we'll flag for doing something wrong if the new song name is too short.
    const ARBETRARY_SONG_MIN_LENGTH: usize = 3;
    ...
    if let Some(remix_start) = query.find("remix") {
        ...
        if let Some(metadata_start) = query.rfind(is_dash) {
            if metadata_start >= ARBETRARY_SONG_MIN_LENGTH {
